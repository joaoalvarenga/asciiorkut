...
class UserModel(object):
    ...
    def save(self):
        if self.id is None:
            with Config().get_db_connection().cursor() as cursor:
                ...
                cursor.execute('INSERT INTO usuarios (email, nome, dataNascimento, sexo, senha, codigoAtor, codigoPublicavel)'
    ...
    def find_friends(uid):
        with Config().get_db_connection().cursor() as cursor:
            cursor.execute('SELECT codigo, email, nome, dataNascimento, sexo, senha, codigoAtor, codigoPublicavel '
                           'FROM usuarios, sao_amigos '
                           'WHERE sao_amigos.codigoUsuario1 = {} AND usuarios.codigo = sao_amigos.codigoUsuario2'
                           .format(uid))
